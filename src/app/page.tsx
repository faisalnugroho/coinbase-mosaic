'use client';
// Mosaic — Final polished version after 4-step self-critique + revision
// 19 issues found, all fixed.
    ...
    <>
      ...
      <main>
        ...
        <section className="relative px-5 pt-16 pb-4 text-center max-w-3xl mx-auto overflow-hidden">
          ...
          <div className="flex items-center justify-center gap-3 mt-7 reveal r2">
            <button onClick={() => setShowLogin(true)} disabled={claiming}
              className="bg-[#0052FF] text-white font-semibold px-7 py-3.5 rounded-full hover:bg-[#0039b3] disabled:opacity-60 disabled:cursor-not-allowed transition-all text-sm glow-sm flex items-center gap-2" aria-label="Connect X to claim your permanent pixel">
              ...
              {hasClaims ? 'Claim Your Pixel' : 'Be the First'}
            </button>
          ...
          <div ref={statsRef} className="flex items-center justify-center gap-10 mt-8 reveal r3">
            {[{ v: claimedCount.toLocaleString(), l: 'Claimed' }, { v: remaining.toLocaleString(), l: 'Available' }, { v: `${pct}%`, l: 'Complete' }].map((s, i) => (
              <div key={i} className="text-center">
                <div className={`text-white text-[26px] font-bold tabular-nums ${statsAnimated ? 'count-up' : 'opacity-0'}`} style={{ animationDelay: `${i * 0.15}s` }}>{s.v}</div>
                <div className="text-[#5c6880] text-[11px] uppercase tracking-[0.12em] font-medium mt-0.5">{s.l}</div>
              </div>
            ))}
          </div>
        </section>
        ...
        <section id="canvas" className="px-3 py-8 max-w-[700px] mx-auto reveal-on-scroll">
          <div className="flex items-center justify-between mb-3 px-1">
            <div className="flex items-center gap-2">
              <span className="relative flex h-2 w-2" aria-hidden="true">
                <span className={`animate-ping absolute inline-flex h-full w-full rounded-full ${hasClaims ? 'bg-[#00b4d8]' : 'bg-[#0052FF]/40'} opacity-75`} />
                <span className={`relative inline-flex rounded-full h-2 w-2 ${hasClaims ? 'bg-[#00b4d8]' : 'bg-[#0052FF]/60'}`} />
              </span>
              <span className={`text-[11px] font-semibold uppercase tracking-wider ${hasClaims ? 'text-[#00b4d8]' : 'text-[#0052FF]/60'}`}>
                {hasClaims ? 'Live' : 'Ready'}
              </span>
          ...
          <div className="relative aspect-square w-full glass overflow-hidden border border-[#0052FF]/6 glow">
        ...
        <section id="activity" className="px-5 py-8 max-w-3xl mx-auto">
          ...
          {hasClaims ? (
            <>
              <div className="space-y-1 mb-1">
                {(showAllFeed ? liveFeed : liveFeed.slice(0, 5)).map((f, i) => (
                  <div key={i} className={`flex items-center gap-3 px-4 py-3 rounded-xl reveal-on-scroll transition-colors ${i % 2 === 0 ? 'bg-white/[0.015]' : ''}`}>
                    ...
                    <span className="text-white/55 text-[13px]"><span className="text-white font-semibold">@{f.username}</span> joined the mosaic</span>
              ...
              {liveFeed.length > 5 && (
                <button onClick={() => setShowAllFeed(!showAllFeed)} className="text-[#5c6880] hover:text-white text-xs transition-colors mt-2 ml-4">{showAllFeed ? 'Show less' : `View all ${liveFeed.length} entries`}</button>
              )}
            </>
          ) : (
            <div className="glass-elevated p-8 text-center reveal-on-scroll border-[#0052FF]/8">
              <div className="text-4xl mb-4" aria-hidden="true">🟦</div>
              <h3 className="text-white font-bold text-lg mb-2">The Canvas Is Empty</h3>
              <p className="text-[#5c6880] text-sm max-w-xs mx-auto mb-5 leading-relaxed">No one has claimed a pixel yet. The first contributors will be remembered as the founders of this digital monument.</p>
              <button onClick={() => setShowLogin(true)} className="inline-flex bg-[#0052FF] text-white font-semibold px-6 py-3 rounded-full hover:bg-[#0039b3] transition-all text-sm glow-sm">Claim the First Pixel</button>
            </div>
          )}
        </section>
        ...
        <section id="how-it-works" className="px-5 py-12 max-w-3xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            {[
              { step: '01', icon: <LinkIcon />, title: 'Connect', desc: 'Sign in with your X account. One click. Your profile loads.' },
              { step: '02', icon: <TargetIcon />, title: 'Choose', desc: 'Find an empty pixel that calls to you. Claim your place.' },
              { step: '03', icon: <SparkIcon />, title: 'Become', desc: 'Your profile becomes internet history. Visible forever.' },
            ].map((s, i) => (
              <div key={i} className="glass p-6 reveal-on-scroll hover:border-[#0052FF]/20 hover:bg-white/[0.025] transition-all duration-500 group relative" style={{ transitionDelay: `${i * 0.1}s` }}>
                ...
                <div className="text-[#0052FF] mb-3 group-hover:text-[#00b4d8] group-hover:scale-110 transition-all duration-300 inline-block">{s.icon}</div>
        ...
        {leaderboard.length > 0 && (
          <section className="px-5 py-10 max-w-3xl mx-auto">
            <h2 className="text-white/70 text-[13px] font-bold uppercase tracking-[0.12em] text-center mb-6 reveal-on-scroll">Early Contributors</h2>
            <div className="flex flex-wrap justify-center gap-2">
              {leaderboard.map((c, i) => (
                <div key={i} className="glass px-3.5 py-2 flex items-center gap-2 reveal-on-scroll hover:border-[#0052FF]/20 transition-all" style={{ transitionDelay: `${i * 40}ms` }}>
                  ...
                  <span className="text-white/55 text-[13px] font-medium">@{c.username}</span>
                </div>
              ))}
            </div>
          </section>
        ...
        <footer className="px-5 py-14 max-w-3xl mx-auto text-center border-t border-white/[0.03]" role="contentinfo">
          <div className="flex items-center justify-center gap-2 mb-5">
            ...
            <span className="text-white/20 text-xs">community mosaic</span>
          </div>
          ...
          <div className="relative w-full sm:max-w-sm mx-4 mb-4 sm:mb-0 glass-elevated p-6 fade-scale z-10" onClick={e => e.stopPropagation()}>
            ...
            <div className="text-center pt-2">
              ...
              <div className="relative inline-block mb-4">
                ...
                <div className="absolute inset-0 rounded-full blur-xl bg-[#0052FF]/10 -z-10" aria-hidden="true" />
              </div>
